...
└── passenger_menu (1-4)
    ├── 1 — בדיקת יתרת נקודות → "יתרה תישלח בהודעה" → hangup
...
13. **מחירון** — ישות `Price` הוסרה; המחירים עברו לשדה `knowledge` ב-BotConfig. כרגע אין מנוע שמחלץ מחיר אוטומטית מהידע — הבוט/מפעיל צריכים לדעת אותו.
